 ...
 * onto the Caint event bus, translating them into plain, Caint-safe payloads.
 *
 * This is the ONLY place in the wrapper that is allowed to touch
 * NotificationCenter directly. Everything downstream of it (Caint.Events
 * listeners) only ever sees primitive data -- never TLRPC objects or other
 * Telegram internals -- which is what keeps Caint.Events safe to expose to
 * future plugin code.
 *
 * To bridge an additional Telegram event, add its id to BRIDGED_IDS and a
 * case to mapEventName -- nothing else needs to change.
 */
final class CaintNotificationBridge implements NotificationCenter.NotificationCenterDelegate {

    private static final CaintNotificationBridge INSTANCE = new CaintNotificationBridge();
    ...
    private CaintNotificationBridge() {
    }
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        ...
        // Intentionally emit only primitive, Caint-safe data (the account
        // index) -- never the raw Telegram objects carried in `args`.
        Caint.Events.emit(eventName, account);
    }
