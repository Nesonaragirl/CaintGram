...
import com.caint.api.CaintComponent;
...
/**
 * Builds the `Caint` table handed to a plugin's Lua state.
 *
 * This is a deliberately curated subset of the real Caint Java API -- only
 ...
 *
 * Container support (Caint.UI.ChatHeader etc) and full permission checks
 * aren't wired in yet -- this is the loading mechanism, not the sandbox.
 */
final class CaintLuaBridge {

    // event name -> (the plugin's lua function -> the CaintCallback we
    // registered for it), so Events.off can find and remove the right one.
    private final Map<String, Map<LuaValue, CaintCallback>> eventCallbacks = new HashMap<>();
    ...
    private LuaTable buildUiTable() {
        ...
        ui.set("createButton", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                return componentToLua(Caint.UI.createButton(text));
            }
        });
        ...
        ui.set("createMenuItem", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                return componentToLua(Caint.UI.createMenuItem(text, icon));
            }
        });
        ...
        ui.set("createDialog", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                return componentToLua(Caint.UI.createDialog(title, content));
            }
        });
    ...
    private LuaTable buildEventsTable() {
        ...
        events.set("on", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                CaintCallback callback = new LuaCallbackAdapter(fn);
                Map<LuaValue, CaintCallback> forEvent = eventCallbacks.get(event);
                if (forEvent == null) {
                    forEvent = new HashMap<>();
                    eventCallbacks.put(event, forEvent);
                }
                forEvent.put(fn, callback);
                Caint.Events.on(event, callback);
                return LuaValue.NIL;
            }
        });
        ...
        events.set("off", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                Map<LuaValue, CaintCallback> forEvent = eventCallbacks.get(event);
                if (forEvent != null) {
                    CaintCallback callback = forEvent.remove(fn);
                    if (callback != null) {
                        Caint.Events.off(event, callback);
                    }
                }
                return LuaValue.NIL;
            }
        ...
        events.set("emit", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                Caint.Events.emit(event, data.isnil() ? null : data);
                return LuaValue.NIL;
            }
        });
    ...
    private LuaTable buildSettingsTable() {
        ...
        settings.set("get", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                Object value = Caint.Settings.get(key);
        ...
        settings.set("set", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                Caint.Settings.set(key, value.isnil() ? null : value.tojstring());
                return LuaValue.NIL;
            }
        });
    ...
    private LuaTable buildDialogsTable() {
        ...
        dialogs.set("confirm", new VarArgFunction() {
            ...
            public Varargs invoke(Varargs args) {
                ...
                Caint.Dialogs.confirm(title, text, fn != null ? new LuaCallbackAdapter(fn) : null);
                return LuaValue.NIL;
            }
        });
    ...
    /**
     * Wraps a CaintComponent as a Lua table exposing the same method names
     * as the Java CaintComponent interface, callable with colon syntax
     ...
     */
    private static LuaTable componentToLua(CaintComponent component) {
